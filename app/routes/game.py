    ...
    try:
        ...
        if existing_session:
            ...
            return GameResponse(
                ...
                bot_response=f"Welcome back! Resuming from Stage {stage}. You have found {len(current_stage_keys_found)}/{len(stage_config['keys'])} keys in this stage. {stage_config['moods'][existing_session['character_mood']]}",
            ...
            return GameResponse(
                ...
                bot_response=f"Welcome to the Prompt Injection Escape Game! {stage_config['story']} {stage_config['moods']['helpful']}",
                extracted_keys=[],
                score=0,
                attempts=0,
                resistance_level=1,
...
async def start_fresh_game(current_user: str = Depends(get_current_user)):
    ...
    try:
        ...
        return GameResponse(
            ...
            character=stage_config["character"],
            character_mood="helpful",
            bot_response=f"Welcome to a fresh Prompt Injection Escape Game! All previous progress has been cleared. {stage_config['story']} {stage_config['moods']['helpful']}",
            extracted_keys=[],
            score=0,
            attempts=0,
            resistance_level=1,
    ...
    try:
        ...
        cursor.execute("""
            UPDATE game_sessions SET
                ...
                game_over = ?, success = ?, updated_at = CURRENT_TIMESTAMP
            WHERE id = ?
        """, (
